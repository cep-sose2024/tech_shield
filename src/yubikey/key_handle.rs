...
use yubikey::{YubiKey, piv::{self, algorithm::AlgorithmId, SlotId}, MgmKey};
use tracing::instrument;
use sha2::Sha256;
use openssl::{hash::MessageDigest, pkey::{PKey, Public}, sign::Verifier, rsa::{Padding, Rsa}, ec::EcKey};
...
impl KeyHandle for YubiKeyProvider {
    ...
    fn encrypt_data(&self, data: &[u8], key: &[u8]) -> Result<Vec<u8>, yubikey::Error> {
        ...
        match self.key_algorithm {
            "Rsa" => {
                let rsa = Rsa::public_key_from_pem(key).map_err(|_| "failed to create RSA from public key PEM");
                let mut encrypted_data = vec![0; rsa.size() as usize];
                rsa.public_encrypt(data, &mut encrypted_data, Padding::PKCS1).map_err(|_| "failed to encrypt data");
    ...
    #[instrument]
    fn verify_signature(&self, data: &[u8], signature: &[u8], pkey: &[u8]) -> Result<bool, SecurityModuleError> {
        
        match self.key_algorithm {
            "Rsa" => {
                let rsa = Rsa::public_key_from_pem(pkey).map_err(|_| "failed to create RSA from public key PEM");
                let key_pkey = PKey::from_rsa(rsa).unwrap();
                
                let mut verifier =
                Verifier::new(MessageDigest::sha256(), &key_pkey).map_err(|_| "failed to create verifier");
                verifier
                    .update(data)
            ...
            "Ecc" => {
                let ecc = EcKey::public_key_from_pem(pkey).map_err(|_| "failed to create ECC from public key PEM");
                let ecc = PKey::from_ec_key(ecc).map_err(|_| "failed to create PKey from ECC");
            
                let mut verifier =
                Verifier::new(MessageDigest::sha256(), &pkey).map_err(|_| "failed to create verifier");
                verifier
                    .update(data)
                    .map_err(|_| "failed to update verifier");
                verifier
