    ...
    loop {
        ...
        match input.to_string().trim() {
            ...
            "1" => {
                ...
                let sym_algorithm = Some(BlockCiphers::Aes(SymmetricMode::Cbc, KeyBits::Bits2048));
                let hash = Some(Hash::Sha2(Sha2Bits::Sha256));
                let key_usages = vec![KeyUsage::SignEncrypt, KeyUsage::Decrypt];

                match hsm_provider.lock().unwrap().create_key(
                    key_id.to_string(),
                    key_algorithm,
                    sym_algorithm,
                    hash,
                    ...
                    Err(e)=>println!("Failed to crate key: {:?}", e),
                }
            }
            //Encrypt
            "2" => {
            ...
            "4" => {
                ...
                match &hsm_provider.unwrap().lock().unwrap().sign_data(){
                    Ok(signature) => println!("Signature: {:?}", signature),
                    Err(e)=>println!("Failed to sign datas: {:?}", e)
                };
            }
            //Verify Signature
            "5" => {
                ...
                match &hsm_provider.unwrap().lock().unwrap().verify_signature(data, &signature){
                    Ok(valid)=>{
                        if valid {
